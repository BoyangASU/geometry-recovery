...
4. :mod:`stfft.geometry`     -- top-down ellipse-fit geometry recovery
5. :mod:`stfft.profile`      -- side-view surface-profile recovery
...
from . import drift, geometry, io, iqa, profile, segmentation, transform
...
def run_pipeline(video: np.ndarray, k: int = 2, k_geom: int = 3, cutoff: float = 30.0, do_drift: bool = True):
    ...
    k:
        Number of intensity clusters for IQA segmentation (2 or 3).
    k_geom:
        Clusters for the final geometry segmentation. The paper uses ``k=3``
        so the part *halo* is separated from the weld nugget for a cleaner
        contour.
    cutoff:
    ...
    # 4. Geometry recovery from the enhanced image (K=3 to peel off the halo).
    enhanced_norm = transform.normalize(enhanced)
    object_map_enh = segmentation.kmeans_threshold(enhanced_norm, k=k_geom)
